...
def get_events(request):
    ...
    for session in sessions:
        ...
        events.append({
            ...
            'allDay': False,
            'backgroundColor': session.hall.color if session.hall and session.hall.color else '#3788d8',
            'borderColor': session.hall.color if session.hall and session.hall.color else '#3788d8',
            'extendedProps': {
                'hall_id': session.hall.id if session.hall else None,
                'hall_name': session.hall.name if session.hall else '',
                'hall_color': session.hall.color if session.hall and session.hall.color else '#3788d8',
